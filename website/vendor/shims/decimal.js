...
(function(global) {
        ...
        for (;;) {
            ...
            if (t['c'].slice(0, sd).join('') === r['c'].slice(0, sd).join('')) {
                ...
                if ((c[sd - 3] == 9 || !rep && c[sd - 3] == 4) &&
                     ...
                     */
                    if ((!c[sd - 3] || c[sd - 3] == 5) && !c[sd - 2] && !c[sd - 1] && !c[sd]) {
    ...
    P['toExponential'] = function(dp, rm) {
        ...
        return format(x, dp != null && checkArg(x, dp, 'toExponential') || !x['c'] ? dp | 0 : x['c'].length - 1, dp != null && checkRM(x, rm, 'toExponential'), 1);
    };
    ...
    P['toFraction'] = function(maxD) {
        ...
        frac = div(n1, d1, e, 1, 1)['minus'](x)['abs']()['cmp'](
            div(n0, d0, e, 1, 1)['minus'](x)['abs']()) < 1 ? [n1.toString(), d1.toString()] : [n0.toString(), d0.toString()];
            ...
            if (!n && s < 0) {
                ...
                if (n && a * i < INT_POW_LIMIT) {
                    ...
                    /*
                       ...
                       */
                    ? Math.floor(yN * (
                        Math.log('0.' + x['c'].join('')) / Math.LN10 + x['e'] + 1)) : new Decimal(b + '')['e'];
    ...
    P['toPrecision'] = function(sd, rm) {
        ...
        return sd != null && checkArg(this, sd, 'toPrecision', 1) ? format(this, --sd | 0, checkRM(this, rm, 'toPrecision'), 2) : this.toString();
    };
    ...
    function checkRoundingDigits(c, i, rm, repeating) {
        ...
        return (!repeating && rm > 3 && c[i] == 4 ||
            ...
            repeating == null && (c[i] == 5 || !c[i]) && !c[i + 1] && !c[i + 2] && !c[i + 3] && !c[i + 4];
    }
    ...
    function checkRM(x, rm, method) {
        ...
        return rm == null || ((outOfRange = rm < 0 || rm > 8) ||
            rm !== 0 && (Decimal['errors'] ? parseInt : parseFloat)(rm) != rm) && !ifExceptionsThrow(Decimal, 'rounding mode', rm, method, 0) ? Decimal['rounding'] : rm | 0;
    }
    ...
    function ifExceptionsThrow(Decimal, message, arg, method, more) {
        ...
        if (Decimal['errors']) {
            var error = new Error((method || [
                ...
                'number type has more than 15 significant digits', 'LN10 out of digits'
            ][message] || message + ([outOfRange ? ' out of range' : ' not an integer',
    ...
    function rnd(x, sd, rm, r, b) {
        ...
        if (sd != rd) {
            ...
            r = rm < 4 ? (rd != null || r) && (rm == 0 || rm == 2 && !isNeg || rm == 3 && isNeg) : rd > half || rd == half && (rm == 4 || r || rm == 6 && xc[sd - 1] & 1 ||
    ...
    DecimalConstructor = (function() {
        ...
        var random = (function() {
            ...
            function getRandomBytes(max) {
                ...
                return max ? (n = rb(1)[0], n > (256 / max | 0) * max - 1 ? getRandomBytes(max) : n % max + '')
            ...
            return function(limit, pr) {
                ...
                // null/+-Infinity/NaN?
                if (one = limit == e || !(id = 14, limit = new Decimal(limit))['c'] && !ifExceptionsThrow(Decimal, 'limit must be finite', limit, 'random')) {
