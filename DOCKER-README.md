...
### References
	...
	8. [Docker Compose](http://resources.codeship.com/hubfs/resources_PDFs/Codeship_Orchestrate_Containers_for_Development_with_Docker_Compose.pdf?t=1456508363953)
	9. [Docker Engine](http://resources.codeship.com/hubfs/resources_PDFs/Codeship_Why_Containers_and_Docker_are_the_Future.pdf?t=1456508363953)
...
### Helper Shell Scripts
	...
	* `sh docker/start/dev` - Starts Docker environment and sets it ready to use
...
### Setup
	* NOTE - make sure to stop all virtualbox instances running prior to installation
	* Install docker dev environment - `sh docker/install && sh docker/setup`
	* OR MANUALLY
	* Install Homebrew - `ruby -e "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install)"`
	* Make sure Homebrew is up to date - `brew uninstall --force brew-cask; brew update`
	* Install virtualbox - `brew cask install virtualbox`
	* Install docker - `brew install docker docker-compose docker-machine`
	* Install fswatch - `brew install fswatch`
	* Install coreutils - `brew install coreutils`
	* Create SSL Certificates if needed - `sh bin/create-ssl-cert.sh`
	* Create secrets.env file from the shared/env/secrets.env.config file
...
### Start Up
	* `sh docker/start/dev`
	* `docker-machine create --driver "virtualbox" --virtualbox-cpu-count 2 --virtualbox-memory 2048 platform-dev --virtualbox-disk-size 102400` - create virtual machine (virtualbox)
	* OR
	* `docker-machine create --driver "parallels" --parallels-cpu-count=2 --parallels-memory=2048 --parallels-disk-size 102400 platform-dev-parallels` - create virtual machine (parallels)
	* THEN
...
### Most Useful Commands
	...
	* `docker run -it platform_website ember generate resource topics`
...
### Docker Compose Commands
	...
	* `docker-compose -f docker-compose/jenkins.yml up -d` - Compose for jenkins
...
### Updating Container While Developing
	* To add a new npm package or a new bower component
		* Add the package and version to package.json or bower.json and stop your dev environment `(ctrl + c)` and restart it `sh docker/start/dev`
...
### Deployment
	#### Setup Deployment Environment
		...
		*  NOTE - ECR login expires after some time so `sh docker/deploy/init` may need to be run after a couple of days
...
### Troubleshooting
	...
	* `docker-machine restart platform-dev; sh docker/start/dev OR sh docker/restart` - If your docker container seems to not be able to connect to the internet
	...
	* If you see `ERROR: Couldn't connect to Docker daemon - you might need to run `docker-machine start default`.` You may just need to rerun 'sh docker/start/dev'
	* `docker-machine regenerate-certs platform-dev` - to regenerate certificates if worst comes to worst
